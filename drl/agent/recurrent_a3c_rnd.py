...
from drl.agent.config import RecurrentA3CRNDConfig
from drl.agent.net import RecurrentA3CRNDNetwork
from drl.agent.trajectory import (RecurrentA3CRNDExperience,
                                  RecurrentA3CRNDTrajectory)
...
@agent_config("Recurrent A3C RND")
class RecurrentA3CRND(Agent):
    def __init__(
        self, 
        config: RecurrentA3CRNDConfig,
        network: RecurrentA3CRNDNetwork,
        trainer: Trainer,
        num_envs: int,
        ...
        self._trajectory = RecurrentA3CRNDTrajectory(self._config.n_steps)
    ...
    def select_action(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        policy_dist_seq, state_value_seq, nonepi_state_value_seq, next_hidden_state = self._network.forward_actor_critic(
            obs.unsqueeze(dim=1),
            self._hidden_state
        )
    ...
    def update(self, exp: Experience) -> Optional[dict]:
        ...
        self._trajectory.add(RecurrentA3CRNDExperience(
            **exp.__dict__,
            rnd_int_reward=rnd_int_reward,
            epi_state_value=self._state_value,
    ...
    def inference_agent(self, num_envs: int = 1, device: Optional[str] = None) -> Agent:
        return RecurrentA3CRNDInference(self._network, num_envs, device or str(self.device))
    ...
    def _train(self):
        ...
        for _ in range(self._config.epoch):
            ...
            for i in range(entire_seq_batch_size // seq_mini_batch_size):
                ...
                # train step
                loss = actor_loss + self._config.critic_loss_coef * critic_loss + rnd_loss - 0.001 * entropy
    ...
    def _compute_adv_target(self, exp_batch: RecurrentA3CRNDExperience):
        """
...
class RecurrentA3CRNDInference(Agent):
    def __init__(self, network: RecurrentA3CRNDNetwork, num_envs: int, device: Optional[str] = None) -> None:
        super().__init__(num_envs, network, device)
