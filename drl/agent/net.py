...
class RecurrentA3CNetwork(RecurrentNetwork):
    """
    Recurrent Advantage Actor Critic (A3C) shared network.
    
    Since it uses the recurrent network, you must consider the hidden state which can acheive the action-observation history.
    
    Note that since A3C uses the Actor-Critic architecure and the parameter sharing, 
    the encoding layer must be shared between Actor and Critic. 
    """
...
class RecurrentA3CRNDNetwork(RecurrentNetwork):
    """
    Recurrent Advantage Actor Critic (A3C) shared network with Random Network Distillation (RND).
    
    Since it uses the recurrent network, you must consider the hidden state which can acheive the action-observation history.
    
    Note that since A3C uses the Actor-Critic architecure and the parameter sharing, 
    the encoding layer must be shared between Actor and Critic. 
    Be careful not to share parameters between A3C and RND networks.
...
class RecurrentSACNetwork(RecurrentNetwork):
    """
    Recurrent Soft Actor-Critic (SAC) shared network.
    
    Since it uses the recurrent network, you must consider the hidden state which can acheive the action-observation history.
    
    Note that since SAC uses the Actor-Critic architecure and the parameter sharing, 
    the encoding layer must be shared between Actor and Critic. 
    """
